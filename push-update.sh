...
updates_dir=/data/banana_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have BananaDroid-VERSION-DATE-TYPE-* as format"
...
# Assume BananaDroid-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.banana.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.banana.updater/databases/updates.db" \
